...
from api.importer.importer.application.assign_product_type_to_line_command import AssignProductTypeToLineCommand
from api.importer.importer.application.bus import Bus
from api.importer.importer.domain.customer import Customer



class AssignClarusProductTypeToLine():
    ...
    def __init__(self, filename):
        ...
        command = AssignProductTypeToLineCommand(
    ...
    def upload(self):
        ...
        for index, row in self.data.iterrows():
            ...
            child = row['product type']
            parent = row['product line']

            if child is not nan and parent is not nan:

                parent_query = self.session.query(autogen_entities.ProductLine)\
                                        .filter_by(name=parent)\
                                        .first()

                child_query = self.session.query(autogen_entities.ProductType)\
...
uploader = AssignClarusProductTypeToLine(filename)
uploader.upload()