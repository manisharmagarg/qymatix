...
from api.importer.importer.application.assign_product_line_to_class_command import AssignProductLineToClassCommand
from api.importer.importer.application.bus import Bus
from api.importer.importer.domain.customer import Customer



class AssignClarusProductLineToClass():
    ...
    def __init__(self, filename):
        ...
        command = AssignProductLineToClassCommand(
    ...
    def upload(self):
        ...
        for index, row in self.data.iterrows():
            ...
            child = row['product line']
            parent = row['product class']

            if child is not nan and parent is not nan:

                parent_query = self.session.query(autogen_entities.ProductClass)\
                                        .filter_by(name=parent)\
                                        .first()

                child_query = self.session.query(autogen_entities.ProductLine)\
...
uploader = AssignClarusProductLineToClass(filename)
uploader.upload()