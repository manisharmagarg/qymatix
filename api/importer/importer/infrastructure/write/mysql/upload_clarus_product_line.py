...
class UploadClarusProductLine():
    ...
    def upload(self):
        ...
        for index, row in self.data.iterrows():
            product_line_name = row['product line']

            if product_line_name is not nan:

                product_line = ProductLine(product_line_name)

                query = self.session.query(autogen_entities.ProductLine)\
                                        .filter_by(name=product_line_name)\
                                        .first()

                if query == None:
                    new = autogen_entities.ProductLine(
...
uploader = UploadClarusProductLine(filename)
uploader.upload()